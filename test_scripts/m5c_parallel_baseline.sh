...
OUTPUT_DIR="/es01/paratera/sce3640/m5c/m5C-UBSseq-0.1/baseline"
...
process_sample() {
    ...
    $samtools view -e "rlen<100000" -h ${SAMPLE_DIR}/${SAMPLE_ID}.mRNA.genome.mapped.sorted.dedup.bam | \
        $hisat_3n_table -p 1 -u --alignments - --ref $REF_GENOME --output-name /dev/stdout --base-change C,T | \
        cut -f 1,2,3,5,7 | bgzip -c > ${SAMPLE_DIR}/${SAMPLE_ID}_unfiltered_uniq.tsv.gz &

    $samtools view -e "rlen<100000" -h ${SAMPLE_DIR}/${SAMPLE_ID}.mRNA.genome.mapped.sorted.dedup.bam | \
        $hisat_3n_table -p 1 -m --alignments - --ref $REF_GENOME --output-name /dev/stdout --base-change C,T | \
